...
/**
 * Create a Stripe Product and Price for a membership tier on the connected account.
 */
...
export async function createMembership(data: {
  ...
  // Only create Stripe product/price if the studio has Stripe Connect configured
  ...
  if (stripeProductId && studio?.stripeAccountId) {
...
export async function updateMembership(
  ...
  // If membership has no Stripe price yet and studio has Stripe Connect, create one
...
export async function createSubscriptionCheckout(
  ...
  if (!membership.stripePriceId) {
    throw new Error("Membership has no Stripe price configured. Sync to Stripe first.");
  }
...
export async function cancelSubscription(
  ...
  if (subscription.stripeSubscriptionId) {
    if (!subscription.membership.studio.stripeAccountId) {
