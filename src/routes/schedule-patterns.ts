 ...
 */
router.post('/preview', isAuthenticated, hasRole(['Admin', 'Manager']), async (req, res) => {
  try {
    const { recurrenceRule, startDate, endDate, startTime, durationHours } = req.body;

    if (!recurrenceRule || !startDate || !startTime || !durationHours) {
      return res.status(400).json({
        error: 'Missing required fields: recurrenceRule, startDate, startTime, durationHours',
      });
    }
...
router.post('/', isAuthenticated, hasRole(['Admin', 'Manager']), async (req, res) => {
  try {
    const { classId, classStepId, recurrenceRule, startDate, endDate, startTime, durationHours, maxStudents, location } = req.body;
    const studioId = req.user!.studioId;

    if (!classId || !recurrenceRule || !startDate || !startTime || !durationHours || !maxStudents) {
      return res.status(400).json({
        error: 'Missing required fields: classId, recurrenceRule, startDate, startTime, durationHours, maxStudents',
      });
    }
 ...
 */
router.post('/:id/generate', isAuthenticated, hasRole(['Admin', 'Manager']), async (req, res) => {
  try {
    const patternId = parseInt(req.params.id);
    const sessions = await scheduleService.generateSessionsFromPattern(patternId);
 ...
 */
router.put('/:id', isAuthenticated, hasRole(['Admin', 'Manager']), async (req, res) => {
  try {
    const patternId = parseInt(req.params.id);
    const { recurrenceRule, startDate, endDate, startTime, durationHours, maxStudents, location } = req.body;
 ...
 */
router.delete('/:id/future-sessions', isAuthenticated, hasRole(['Admin', 'Manager']), async (req, res) => {
  try {
    const patternId = parseInt(req.params.id);
    const { fromDate } = req.body;

    if (!fromDate) {
      return res.status(400).json({ error: 'Missing required field: fromDate' });
    }

    const result = await scheduleService.deleteFutureSessions(patternId, new Date(fromDate));
 ...
 */
router.put('/sessions/:id', isAuthenticated, hasRole(['Admin', 'Manager']), async (req, res) => {
  try {
    const sessionId = parseInt(req.params.id);
    const { sessionDate, startTime, endTime, maxStudents, location, notes, isCancelled } = req.body;
 ...
 */
router.post('/sessions/:id/cancel', isAuthenticated, hasRole(['Admin', 'Manager']), async (req, res) => {
  try {
    const sessionId = parseInt(req.params.id);
